...
class Leddam(nn.Module):
    def __init__(self,
        ...
        #self.filter = Block(dim=d_model)

    def forward(self, inp):
        inp=self.position_embedder(inp.permute(0,2,1)).permute(0,2,1)
        #inp = self.filter(inp.permute(0,2,1)).permute(0,2,1)
        main=self.LD(inp)
        residual=inp-main
...
class Leddam_model(nn.Module):
    def __init__(self, configs):
        super(Leddam_model, self).__init__()
        ...
        self.leddam=Leddam(configs.enc_in,configs.seq_len,configs.d_model,
                       configs.dropout,configs.pos_embed,kernel_size=25,n_layers=configs.e_layers)
